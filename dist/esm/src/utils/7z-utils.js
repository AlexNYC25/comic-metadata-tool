...
export async function get7zContentList(filePath) {
...
export async function extract7zEntryToTemp(archivePath, entryName, destDir) {
...
export async function does7zContainXml(filePath) {
    const files = await get7zContentList(filePath);
    return files.some((file) => file.endsWith(".xml"));
}
export async function does7zContainJson(filePath) {
    const files = await get7zContentList(filePath);
    return files.some((file) => file.endsWith(".json"));
}
export async function getXmlFilesFrom7z(filePath) {
    const files = await get7zContentList(filePath);
    return files.filter((file) => file.endsWith(".xml"));
}
export async function getJsonFilesFrom7z(filePath) {
    const files = await get7zContentList(filePath);
 ...
 */
export async function get7zEntryContent(archivePath, entryName) {
...
async function extract7zArchiveToDirectory(archivePath, outputDir) {
...
async function create7zArchiveFromDirectory(sourceDir, outputArchivePath) {
    await execFileAsync(sevenBin.path7za, ["a", "-t7z", "-y", "-bd", outputArchivePath, "."], {
        cwd: sourceDir,
    });
}
...
export async function upsert7zEntryContent(archivePath, entryName, content) {
